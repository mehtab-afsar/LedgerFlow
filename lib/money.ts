 ...
 * number, which JavaScript parses as a float. `0.1 + 0.2 !== 0.3`, so an
 * invoice total drifts by a paisa and an accountant eventually finds it.
 *
 * RULE: no arithmetic on a rupee value happens outside this file. lib/tax.ts
 * and every balance calculation take `...Paise: number` specifically so that
