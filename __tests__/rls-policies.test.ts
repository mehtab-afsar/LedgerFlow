 ...
 * organisation, exempt from the tenancy rules below and required to be
 * strictly read-only in exchange.
 *
 * EMPTY here. Every table in this schema holds tenant data, so every policy
 * must fence rows to the caller's organisation, with no exceptions. Ported
 * from Moonbook, where the signup industry catalogue is genuinely shared —
 * that entry is removed rather than carried across, because an allowlist
 * naming something that does not exist is how an allowlist stops meaning
 * anything.
 */
